...
//while($result=mysqli_fetch_array($query,MYSQLI_ASSOC))
//$result["book_timeslot"]
//mysqli_result($query,$run,'$result["book_timeslot"]'
//mysqli_num_rows($query)>$run &&
  ...
  foreach($arrTime as $time => $time_value){
    if($time == $result["book_timeslot"]){//ถ้ามีการจอง
      ...
      $strTable .='<td><a href="booking_form.php?room='.$room.'&time='.$time.'&today='.$today.'" class="btn btn-lg btn-block btn-info" role="button">Available</a></td>';
    }
  }
  } // END Foreach
